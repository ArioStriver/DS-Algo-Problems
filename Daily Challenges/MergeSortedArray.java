...
class Solution {
    public void merge(int[] nums1, int m, int[] nums2, int n) {
        ...
        while(i >= 0 && j >= 0){
            ...
            // then shift the ith element by one place towards right
            if(nums1[i] > nums2[j]){
                nums1[k] = nums1[i];
                i--;k--;
            }
            // if j > i , then merge the j th element in the nums1 array
            else{
                nums1[k] = nums2[j];
                j--;k--;
            }
        ...
        // if there are elemnets remaining in the nums2 array
        while(j >= 0){
            nums1[k] = nums2[j];
            k--;j--;
        }
